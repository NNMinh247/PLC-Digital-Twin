...
| `WiringGameMode.h/.cpp` | `AWiringGameMode` | Đặt `PlayerControllerClass = AWiringPlayerController`; `DefaultPawnClass = AFixedViewPawn` |
| `FixedViewPawn.h/.cpp` | `AFixedViewPawn` | Pawn xem **cố định** (kế thừa DefaultPawn, tắt binding WASD/chuột) — digital twin không di chuyển |
| `PlcLight.h/.cpp` | `APlcLight` | Đèn Y: **tắt = tàng hình, bật = quả cầu sáng ĐỎ** + point light; tự gán `EmissiveMeshMaterial` (khỏi gán material tay); nhãn **H1–H8** trong editor theo `LightIndex` (0..7) |
| `PlcLinkActor.h/.cpp` | `APlcLinkActor` | **WebSocket client**: parse `{x,y,d}`; events `OnInputChanged`/`OnLightChanged`/`OnRegisterChanged`; `ToggleLight()` gửi `{toggle:n}`; **giả lập: phím `S` bật/tắt, `1–8` toggle đèn tại chỗ + log màn hình**; map index→`APlcLight` |
...
- Toàn bộ class C++ + `WiringInteraction.h` (+ `FixedViewPawn` camera cố định).
...
- **Đèn: tàng hình khi tắt / sáng đỏ khi bật** (tự dùng `EmissiveMeshMaterial`, nhãn H1–H8) + **giả lập bật đèn** (phím `S`, `1–8`) — xem [`BAN_GIAO.md`](./BAN_GIAO.md).

**⏳ Chưa làm (cần người tiếp tục — xem `BAN_GIAO.md` / `VIEC_CAN_LAM_TIEP.md`):**
- **Test với PLC thật** qua bridge (`{x,y,d}`) — mới chỉ chạy giả lập chiều `y`.
- Tạo **`WBP_HMI`** (parent `UHmiWidget`) ở `/Game/UI` + dựng layout 4 ô (log giả lập hiện dùng on-screen debug).
- Đặt **2 `AHmiCaptureActor`** trong level (`View` = Board16x9 ngắm bàn dây, Lights16x3 ngắm đèn).
- **Commit asset** đang dở (HDRI + external actors của Map).
...
- [`BAN_GIAO.md`](./BAN_GIAO.md) — **bàn giao mới nhất** (tín hiệu đèn: đỏ/tàng hình, giả lập, camera cố định).
