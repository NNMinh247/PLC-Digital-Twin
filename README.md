...
| `HmiWidget.h/.cpp` | `UHmiWidget` | Lớp nền cho `WBP_HMI`: giữ 2 render target, nghe event PLC, format log/status; expose 4 `BlueprintImplementableEvent` |
...
- Build C++ trong VS/editor (chưa xác nhận build sạch trên máy đích).
- **Migrate asset** (model PLC, map, `IA_Grab`, `IMC_Wiring`, `M_Wire_Green/Red`) — giữ đúng đường dẫn ở mục Cạm bẫy.
- Tạo **`WBP_HMI`** (parent `UHmiWidget`) ở `/Game/UI` + dựng layout 4 ô.
- Đặt **2 `AHmiCaptureActor`** trong level.
...
- **HMI là màn hình GIÁM SÁT:** 2 ô trái là ảnh từ SceneCapture (không tương tác). Khi HUD phủ full màn sẽ che thao tác kéo dây bằng chuột — đây là 2 chế độ khác nhau.
- **.NET:** UE 5.6 cần .NET 8. Máy có .NET 9/10 → lỗi `Microsoft.Build/Microsoft.IO.Redist restored using .NETFramework…`, `exited with code 6`. Khắc phục: cài .NET 8 SDK; nếu vẫn lấy nhầm thì thêm `global.json` (`{"sdk":{"version":"8.0.0","rollForward":"latestMinor"}}`) ở gốc project. **(Hiện project PLC chưa có `global.json`.)**
